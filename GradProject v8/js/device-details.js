    ...
    try {
        ...
        if (deviceDoc.exists) {
            ...
            document.getElementById('currentConsumption').textContent = 
                deviceData.currentConsumption ? `${deviceData.currentConsumption.toFixed(2)} KW` : '0.00 KW';
            document.getElementById('todayUsage').textContent = 
                deviceData.todayUsage ? `${deviceData.todayUsage.toFixed(2)} KW` : '0.00 KW';
            document.getElementById('monthlyUsage').textContent = 
                deviceData.monthlyUsage ? `${deviceData.monthlyUsage.toFixed(2)} KW` : '0.00 KW';
            
            // Update cost information
            document.getElementById('currentRate').textContent = 
                deviceData.currentRate ? `${deviceData.currentRate.toFixed(2)} SAR/kWh` : '0.00 SAR/kWh';
            document.getElementById('todayCost').textContent = 
                deviceData.todayCost ? `${deviceData.todayCost.toFixed(2)} SAR` : '0.00 SAR';
            document.getElementById('monthlyCost').textContent = 
                deviceData.monthlyCost ? `${deviceData.monthlyCost.toFixed(2)} SAR` : '0.00 SAR';
...
function checkPowerLimits(deviceData) {
    ...
    const currentConsumption = deviceData.currentConsumption || 0;
    const todayUsage = deviceData.todayUsage || 0;
    const monthlyUsage = deviceData.monthlyUsage || 0;
    const todayCost = deviceData.todayCost || 0;
    const monthlyCost = deviceData.monthlyCost || 0;
